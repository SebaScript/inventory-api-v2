...
/**
 * Creates the whole schema as plain SQL, so every constraint is visible and
 * `down` is complete: the database can always be rebuilt from zero.
 */
export class InitSchema1755300000000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    // Case-insensitive uniqueness: "Tools" and "tools" are the same category.
    await queryRunner.query(`CREATE UNIQUE INDEX ux_groups_name ON groups (lower(name))`);

    await queryRunner.query(`CREATE TYPE item_status AS ENUM ('ACTIVE', 'DISCONTINUED')`);
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`DROP TYPE IF EXISTS item_status`);
